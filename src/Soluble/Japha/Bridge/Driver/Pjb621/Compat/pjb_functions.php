...
{
    return Pjb621\PjbProxyClient::getInstance()->getClient();
}
...
{
    return Pjb621\PjbProxyClient::getInstance()->autoload5($x);
}
...
{
    return Pjb621\PjbProxyClient::getInstance()->autoload($x);
}
...
{
    return Pjb621\PjbProxyClient::getInstance()->getJavaClass($name);
}
 ...
 *
 * @deprecated
 * @param Pjb621\JavaType $object A java object or type
 * @param string $method A method string
 * @param array $args An argument array
 */
function java_invoke($object, $method, $args)
{
    Pjb621\PjbProxyClient::getInstance()->invokeMethod($object, $method, $args);
}
...
{
    return Pjb621\PjbProxyClient::getInstance()->getCompatibilityOption($client);
}
...
/**
 *
 * @param Pjb621\JavaType $ob
 * @param Pjb621\JavaType $clazz
 * @return boolean
 */
function java_instanceof_internal(Pjb621\JavaType $ob, Pjb621\JavaType $clazz)
{
    return Pjb621\PjbProxyClient::getInstance()->isInstanceOf($ob, $clazz);
}
 ...
 * @deprecated
 * @see java_closure()
 * @param Pjb621\JavaType $object
 */
function java_values(Pjb621\JavaType $object)
{
    return Pjb621\PjbProxyClient::getInstance()->getValues($object);
}
 ...
 * @return string
 */
function java_inspect_internal(Pjb621\JavaType $object)
{
    //$client = __javaproxy_Client_getClient();
    //return $client->invokeMethod(0, "inspect", array($object));
    return Pjb621\PjbProxyClient::getInstance()->inspect($object);
}
 ...
 * @throws Exception\IllegalArgumentException
 */
function java_inspect(Pjb621\JavaType $object)
{
    return Pjb621\PjbProxyClient::getInstance()->inspect($object);
...
{
    return Pjb621\PjbProxyClient::getInstance()->getLastException();
}
...
{
    return Pjb621\PjbProxyClient::getInstance()->clearLastException();
}
...
function java_call_with_continuation($kontinuation = null)
{
    if (java_getHeader("X_JAVABRIDGE_INCLUDE", $_SERVER) && !java_getHeader("X_JAVABRIDGE_INCLUDE_ONLY", $_SERVER)) {
        ...
        } elseif ($kontinuation instanceof Pjb621\JavaType) {
 ...
 * @param Pjb621\JavaType $object
 */
function java_unwrap(Pjb621\JavaType $object)
{
    $client = __javaproxy_Client_getClient();
 ...
 * @param Pjb621\JavaType $object
 * @param mixed $type
 * @return Pjb621\JavaType
 */
function java_cast_internal($object, $type)
{
    if (!$object instanceof Pjb621\JavaType) {
        switch ($type[0]) {
            case 'S': case 's':
...
{
    if (isset($args[2]) && ((!($args[2] instanceof Pjb621\JavaType)) && !is_array($args[2]))) {
