...
class DriverManager
{
	...
	{
		if ($conn === null) $conn = Doctrine_Manager::getInstance()->getCurrentConnection();
		if ($driverType != 'mysql') throw new Vision_Exception("Currently only mysql is supported");
		$opts = new Vision_Doctrine_Connection_Options($conn);
		return self::getJdbcDsn($opts->getDatabase(), $opts->getHost(),
								$opts->getUsername(), $opts->getPassword(),
								$driverType);
